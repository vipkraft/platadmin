...
begin
  ...
  If flwait then
   ...
   //не получилось подключиться с первого раза открываем окно
    FormIdle:=TFormIdle.create(FormMain);
    FormIdle.Height:=160;
    FormIdle.width:=formMain.Width;
    FormIdle.Left:=formMain.Left;
    FormIdle.top:=formMain.top+(formMain.Height div 2)-(FormIdle.Height div 2);
    FormIdle.Show;
 ...
 // Формат 0.00
  if dec=2 then
    begin
      ...
      if trim(copy(trim(cislo),1,pos('.',trim(cislo))-1))='' then
          begin
           cislo:='0'+trim(cislo);
          end;
    end;
...
begin
   ...
   numcol :=0; //текстовая
   try
    numcol:= strtoint(grid.Cells[col,1]); //проверка числовой колонки
   except
    numcol :=0;
   end;
