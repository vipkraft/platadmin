...
begin
  ...
  if not(trim(form6.Edit1.text)='') and not(trim(form6.Edit2.text)='') then
    ...
    begin
      showmessagealt('Введены не все добавляемые параметры !');
    end;
end;
...
begin
 ...
 try
  ...
  form6.ZQuery1.SQL.add('UPDATE av_arm SET del=1 WHERE del=0;');
...
begin
  ...
  idarm:=form6.stringgrid1.Cells[1,form6.stringgrid1.row];
  form6.Close;
end;
...
begin
  ...
  try
   form6.ZQuery1.SQL.Clear;
   form6.ZQuery1.SQL.add('SELECT * FROM av_arm WHERE del=0 ORDER BY id ASC;');
   form6.ZQuery1.open;
  except
