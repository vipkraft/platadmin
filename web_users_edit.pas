...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, Buttons, EditBtn, ExtDlgs, unix, DB, LCLProc,
...
begin
 ...
 //если режим добавления
   ...
   //Запрос
     ZQuery1.SQL.Clear;
     ZQuery1.SQL.add('SELECT a.id,b.name,a.point_id FROM av_servers AS a, av_spr_point AS b WHERE a.del=0 AND b.del=0 AND a.point_id=b.id ORDER BY b.name;');
     end;
   //если режим редактирования
   If fl_open=2 then
     begin
   ...
   //Запрос
     ZQuery1.SQL.Clear;
     ZQuery1.SQL.add('SELECT a.id,b.name,a.point_id,CASE WHEN c.server_id is null THEN 0 ELSE 1 END flag FROM av_servers a ');
     ZQuery1.SQL.add('JOIN av_spr_point b ON a.del=0 AND b.del=0 AND a.point_id=b.id ');
     ZQuery1.SQL.add('LEFT JOIN av_users_servers c ON c.del=0 AND c.server_id=a.id AND c.user_id='+nuser);
     ZQuery1.SQL.add(' WHERE a.del=0 AND a.id not in (SELECT denyuser_id FROM av_servers_denyuser WHERE del=0 AND denyuser_id='+nuser+') ORDER BY flag DESC,b.name ASC; ');
     end;
   ...
   begin
     ...
     IF fl_open=1 then
       Stringgrid2.Cells[2,Stringgrid2.RowCount-1] := '0'
         ...
         Stringgrid2.Cells[2,Stringgrid2.RowCount-1] := ZQuery1.FieldByName('flag').asString;
      ZQuery1.Next;
     end;
...
begin
 ...
 If (resid=0) or (trim(resstr)='') then exit;
  ...
  ZQuery1.SQL.Clear;
  ZQuery1.SQL.add('SELECT distinct * FROM av_1c_kontr WHERE kod1c='+inttostr(resid)+' AND polname='+quotedstr(resstr)+';');
  //showmessage(ZQuery1.SQL.text);
    try
      ZQuery1.open;
    except
  ...
  try
   If not Zconnection1.InTransaction then
     ...
     begin
      ...
      exit;
       ...
       begin
        ZQuery1.SQL.Clear;
        ZQuery1.Sql.Add('Select max(id) from av_users;');
        ZQuery1.Open;
   ...
   //Если режим редактирования, сначала помечаем запись на удаление
    If fl_open=2 then
     begin
         ZQuery1.SQL.Clear;
         ZQuery1.SQL.add('UPDATE av_users_group SET del=1,createdate=DEFAULT,id_user='+GLuser+' WHERE del=0 AND user_id='+nuser+';');
         ZQuery1.ExecSQL;
         ZQuery1.SQL.Clear;
         ZQuery1.SQL.add('UPDATE av_users_servers SET del=1,createdate=DEFAULT,id_user='+GLuser+' WHERE del=0 AND user_id='+nuser+';');
         ZQuery1.ExecSQL;
         ZQuery1.SQL.Clear;
         ZQuery1.SQL.add('UPDATE av_users_podr SET del=1,createdate=DEFAULT,id_user='+GLuser+' WHERE del=0 AND user_id='+nuser+';');
         ZQuery1.ExecSQL;
         ZQuery1.SQL.Clear;
         ZQuery1.SQL.add('UPDATE av_users SET del=1,status=0,createdate=DEFAULT,id_user='+GLuser+' WHERE del=0 AND id='+nuser+';');
         ZQuery1.ExecSQL;
     end;
   ...
   ZQuery1.SQL.add(nuser+','+GLuser+',DEFAULT,');//id
